...
contract("ChainList", function (accounts) {
  ...
  it("should throw an exception if you try to buy an article that does not exist", function () {
    return ChainList.deployed()
      ...
      .then(function (data) {
        ...
        assert.equal(data[5], web3.utils.toWei(`${articlePrice}`, "ether"), "article price must be " + web3.utils.toWei(`${articlePrice}`, "ether"));
      });
  });
  ...
  it("should throw an exception if you try to buy your own article", function () {
    return ChainList.deployed()
      .then(function (instance) {
        ...
        return chainListInstance.buyArticle(1, { from: seller, value: web3.utils.toWei(`${articlePrice}`, "ether") });
      })
      .then(assert.fail)
      .catch(function (error) {
        assert(true);
  ...
  it("should throw an exception if you try to buy an article that has already been sold", function () {
    return ChainList.deployed()
      .then(function (instance) {
        chainListInstance = instance;
        return chainListInstance.buyArticle(1, { from: buyer, value: web3.utils.toWei(`${articlePrice}`, "ether") });
      })
      .then(function () {
        return chainListInstance.buyArticle(1, { from: web3.eth.accounts[0], value: web3.utils.toWei(`${articlePrice}`, "ether") });
      })
      .then(assert.fail)
      .catch(function (error) {
        assert(true);
