...
contract('ChainList', function (accounts) {
   ...
   it("should let us sell a first article", function () {
      return ChainList.deployed().then(function (instance) {
         ...
         assert.equal(receipt.logs[0].args._price, web3.utils.toWei(`${articlePrice1}`, "ether"), "event article price must be " + web3.utils.toWei(`${articlePrice1}`, "ether"));
         ...
         assert.equal(data[5], web3.utils.toWei(`${articlePrice1}`, "ether"), "article price must be " + web3.utils.toWei(`${articlePrice1}`, "ether"));
      });
   });
   ...
   it("should let us sell a second article", function () {
      return ChainList.deployed().then(function (instance) {
         ...
         assert.equal(receipt.logs[0].args._price, web3.utils.toWei(`${articlePrice2}`, "ether"), "event article price must be " + web3.utils.toWei(`${articlePrice2}`, "ether"));
         ...
         assert.equal(data[1], 2, "article id must be 2");
         ...
         assert.equal(data[5], web3.utils.toWei(`${articlePrice2}`, "ether"), "article price must be " + web3.utils.toWei(`${articlePrice2}`, "ether"));
      });
   });
   ...
   it("should buy an article", function () {
      return ChainList.deployed().then(async function (instance) {
         ...
         const sellerBalance = await web3.eth.getBalance(seller)
         const buyerBalance = await web3.eth.getBalance(buyer)
         sellerBalanceAfterBuy = web3.utils.fromWei(`${sellerBalance}`, "ether");
         buyerBalanceAfterBuy = web3.utils.fromWei(`${buyerBalance}`, "ether");

         // check the effect of buy on balances of buyer and seller, accounting for gas
         assert(Number(sellerBalanceAfterBuy) == (Number(sellerBalanceBeforeBuy) + articlePrice1), "seller should have earned " + articlePrice1 + " ETH");
         assert(Number(buyerBalanceAfterBuy) <= (Number(buyerBalanceBeforeBuy) - articlePrice1), "buyer should have spent " + articlePrice1 + " ETH");

         return chainListInstance.getArticlesForSale();
      }).then(function (data) {
         assert.equal(data.length, 1, "there should now be only 1 article left for sale");
         assert.equal(data[0], 2, "article 2 should be the only article left for sale");

         return chainListInstance.getNumberOfArticles();
      }).then(function (data) {
         assert.equal(data, 2, "there should still be 2 articles in total");
      });
   });
});
